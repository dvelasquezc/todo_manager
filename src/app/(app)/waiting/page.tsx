...
export default async function WaitingPage({ searchParams }: WaitingPageProps) {
  ...
  // Get waiting tasks with pagination
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {tasks?.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p>No tasks waiting</p>
          <p className="text-sm">Tasks waiting for external input will appear here</p>
        </div>
      ) : (
        <TaskList
          tasks={tasks || []}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-4">
          {hasPrevPage ? (
            <Link
              href={`/waiting?page=${currentPage - 1}`}
