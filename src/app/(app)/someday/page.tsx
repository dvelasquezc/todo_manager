...
interface SomedayPageProps {
  searchParams: Promise<{ page?: string }>
}

export default async function SomedayPage({ searchParams }: SomedayPageProps) {
  ...
  // Get someday tasks with pagination
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {tasks?.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          <CloudSun className="h-12 w-12 mx-auto mb-4 opacity-50" />
          <p>No someday tasks</p>
          <p className="text-sm">Tasks for the future will appear here</p>
        </div>
      ) : (
        <TaskList
          tasks={tasks || []}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-4">
          {hasPrevPage ? (
            <Link
              href={`/someday?page=${currentPage - 1}`}
