...
import { getLocalDateString, getLocalDayOfWeek } from '@/lib/timezone'
...
export interface TotalHoursByProject {
  projectId: string
  projectName: string
  projectColor: string
  totalHours: number
...
function generateLocalDateRange(startDate: Date | null, endDate: Date, timezone: string): string[] {
  ...
  // Get start and end dates in user's timezone
  const startStr = getLocalDateString(startDate.toISOString(), timezone)
  const endStr = getLocalDateString(endDate.toISOString(), timezone)

  // Parse the YYYY-MM-DD strings and iterate through calendar dates
  const dates: string[] = []
  const [startY, startM, startD] = startStr.split('-').map(Number)
  const [endY, endM, endD] = endStr.split('-').map(Number)

  // Use UTC to avoid any timezone shifts during iteration
...
// Generate all dates from startDate to endDate as "YYYY-MM-DD" in UTC
function generateUTCDateRange(startDate: Date | null, endDate: Date): string[] {
  if (!startDate) return []
...
// Get total hours by project for the selected time range
export async function getTotalHoursByProject(weeks: number = 4, projectIds?: string[]): Promise<{ data: TotalHoursByProject[] | null; error: string | null }> {
  const supabase = await createClient()
  ...
  for (const session of sessions) {
    ...
    const current = projectTotals.get(projectId) || 0
    projectTotals.set(projectId, current + (session.duration_minutes || 0))
  }

  // Convert to array with project info
  const result: TotalHoursByProject[] = []
  for (const [projectId, minutes] of projectTotals) {
    const project = projectMap.get(projectId)
    result.push({
      projectId,
      projectName: project?.name || 'Unknown',
...
export async function getContextSwitchingTrend(weeks: number = 4, projectIds?: string[]): Promise<{ data: ContextSwitchingDay[] | null; error: string | null }> {
  ...
  // Get user's timezone for proper date grouping
  const timezone = await getUserTimezoneFromProfile(supabase, user.id)
  ...
  for (const session of sessions) {
    ...
    const day = getLocalDateString(session.started_at, timezone)
  ...
  // Generate complete date range to fill gaps with zeros
  const today = new Date()
  let allDateRange = generateLocalDateRange(startDate, today, timezone)

  // For "all time" (startDate is null), derive range from earliest data point
  if (allDateRange.length === 0) {
    const dataKeys = [...dailyData.keys()]
    if (dataKeys.length > 0) {
      dataKeys.sort()
      const earliest = new Date(dataKeys[0] + 'T00:00:00')
      allDateRange = generateLocalDateRange(earliest, today, timezone)
    }
  }

  // Ensure "today" in user's timezone is included (handles server/user timezone mismatch)
  const todayStr = getLocalDateString(today.toISOString(), timezone)
  if (allDateRange.length > 0 && !allDateRange.includes(todayStr)) {
    allDateRange.push(todayStr)
    allDateRange.sort()
  }

  // Build result from full date range, defaulting to 0 for missing days
  const result: ContextSwitchingDay[] = []
  if (allDateRange.length > 0) {
    for (const day of allDateRange) {
      result.push({
        day,
        projectCount: dailyData.get(day)?.size ?? 0,
      })
    }
  } else {
...
export async function getDailyTaskCompletion(weeks: number = 4, projectIds?: string[]): Promise<{ data: DailyCompletion[] | null; error: string | null }> {
  ...
  // Aggregate tasks by date (using user's timezone)
  const dailyTasks = new Map<string, number>()
  for (const task of tasks || []) {
    // Convert UTC completed_at to local date in user's timezone
    const date = getLocalDateString(task.completed_at!, timezone)
  ...
  for (const session of sessions || []) {
    ...
    // Convert UTC started_at to local date in user's timezone
    const date = getLocalDateString(session.started_at, timezone)
  ...
  // For "all time" (startDate is null), derive range from earliest data point
  if (allDateRange.length === 0) {
    const dataKeys = [...dailyTasks.keys(), ...dailyMinutes.keys()]
    if (dataKeys.length > 0) {
      dataKeys.sort()
      const earliest = new Date(dataKeys[0] + 'T00:00:00')
      allDateRange = generateLocalDateRange(earliest, today, timezone)
  ...
  // Use full date range if available, otherwise fall back to data-only dates
  const allDates = allDateRange.length > 0
    ? allDateRange
    : [...new Set([...dailyTasks.keys(), ...dailyMinutes.keys()])]

  // Convert to array and sort
  const result: DailyCompletion[] = allDates
    .map(date => ({
      date,
...
export async function getProductivityByDayOfWeek(weeks: number = 8, projectIds?: string[]): Promise<{ data: DayOfWeekStats[] | null; error: string | null }> {
  ...
  // Get user's timezone for proper date/day grouping
  const timezone = await getUserTimezoneFromProfile(supabase, user.id)
  ...
  for (const session of sessions || []) {
    ...
    // Convert UTC timestamp to local date and day of week in user's timezone
    const dateKey = getLocalDateString(session.started_at, timezone)
    const dayOfWeek = getLocalDayOfWeek(session.started_at, timezone)
  ...
  for (const task of tasks || []) {
    // Convert UTC timestamp to local date and day of week in user's timezone
    const dateKey = getLocalDateString(task.completed_at!, timezone)
    const dayOfWeek = getLocalDayOfWeek(task.completed_at!, timezone)
  ...
  // Calculate averages (only counting days with actual activity)
  const dayNames = ['', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
  const result: DayOfWeekStats[] = []

  for (let i = 1; i <= 7; i++) {
    const data = dayData.get(i)!
    const dayCount = data.daysCount.size || 1
